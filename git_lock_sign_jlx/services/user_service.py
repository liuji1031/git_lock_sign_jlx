...
class UserService:
    ...
    def _get_git_config_direct(self) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if name_result.returncode == 0 and email_result.returncode == 0:
                ...
                if name and email:
                    logger.info("Found git user config: %s <%s>", name, email)
            ...
            if name_result.returncode == 0 and email_result.returncode == 0:
                ...
                if name and email:
                    logger.info("Found global git user config: %s <%s>", name, email)
                    return {"name": name, "email": email}
            ...
            logger.error("Error executing git config: %s", str(e))
            return None
    ...
    def _get_global_git_config(self) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            logger.debug("Could not read global git config: %s", str(e))
            return None

    def _get_local_git_config(self) -> Optional[Dict[str, Any]]:
        """Get user info from local repository git configuration (legacy method)."""
        try:
            # Try to find a git repository in current working directory or parent
            # directories
            ...
            logger.debug("Could not read local git config: %s", str(e))
            return None
        except Exception as e:
            logger.error("Unexpected error reading local git config: %s", str(e))
            return None
